    ...
    if (data) {
      if (data.channelId === message.channel.id) {
        ...
        if (
          ...
          return message.channel
            .send({
              content: `<:Crossmark:1447578705198055484> | Don't use any of my commands here.`,
            })
            .then((x) => {
              setTimeout(() => {
                x.delete().catch((err) =>
        ...
        if (result.type === "PLAYLIST") {
          const embed = new EmbedBuilder()
            .setDescription(
              `<:queue:1447944310619111455> | **Queued** ${result.tracks.length} **from [${result.playlistName}](${client.config.ssLink})**`
            )
            .setColor(client.color);
          ...
          const embed = new EmbedBuilder()
            .setColor(client.color)
            .setDescription(
              `<:queue:1447944310619111455> | **Queued [${
